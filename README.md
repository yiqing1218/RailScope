...
地铁与国铁运行菜单、计划和时钟独立。国铁按车次选择独立时刻表 / 运行图，一车次对应一列车；工作台可手动新增车次，运行菜单可批量导入严格 CSV 表。共享径路 JSON v2 避免每个车次重复声明轨道；兼容 v1。变道信息预留，不表示已验证真实股道。铁路分为全国业务线路目录和省市车站目录；每条地图轨道保留端点图属性。地图菜单可按类型设置颜色 / 线宽。
...
- 国铁分为线路目录和车站目录。线路目录按高速、普速、城际、市域/市郊和其他铁路的业务层级列出整条线路；车站目录按省、市列出车站实体、线路所和道岔。底层轨道仍在真实拓扑端点处分段，但 RS 不作为目录选择项；通道通过“端点—线路—端点”逐步组合。全国目录按需限制 Qt 树节点数，完整端点图由磁盘索引承担。
