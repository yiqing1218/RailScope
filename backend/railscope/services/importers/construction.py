...
def is_construction_metro_tags(tags: dict[str, str]) -> bool:
    ...
    if railway == "construction" and construction in _URBAN_CONSTRUCTION:
        return True
...
def _is_construction_route(tags: dict[str, str]) -> bool:
    return tags.get("type") == "route" and tags.get("route") in SUPPORTED_ROUTES and (
...
def extract_construction_metro(pbf_path: Path, output_dir: Path) -> ConstructionMetroImportResult:
        ...
        def relation(self, relation: Any) -> None:
            ...
            for sequence, member in enumerate(relation.members):
                ...
                if str(member.type) == "w":
                    self.way_routes.setdefault(int(member.ref), []).append(int(relation.id))
            self.routes[int(relation.id)] = route_record(int(relation.id), tags, members)
    ...
    class WayHandler(osmium.SimpleHandler):
        ...
        def way(self, way: Any) -> None:
            ...
            self.features.append({
                ...
                "properties": {
                    "osm_way_id": way_id,
                    "route_relation_ids": route_ids,
                    "line_key": inherited["line_key"] if inherited else f"osm-way-{way_id}",
                    "line_name": (inherited["name"] if inherited else tags.get("name") or tags.get("name:zh") or tags.get("ref") or "未命名在建城市轨道"),
                    ...
                    "route": inherited["route"] if inherited else tags.get("construction") or tags.get("railway"),
    ...
    way_scan = WayHandler()
    try:
        way_scan.apply_file(str(pbf_path), locations=True, idx=f"sparse_file_array,{index}")
    ...
    _atomic_json_write(geojson_path, {"type": "FeatureCollection", "name": "china_osm_construction_metro", "features": features})
    _atomic_json_write(manifest_path, {
        ...
        "scope": "Explicit OSM construction subway/light-rail route relations, plus ways tagged railway=construction and construction in {subway, light_rail, metro}; raw relation and way tags retained per feature.",
    })
    warnings = []
    if invalid_way_ids:
        warnings.append(f"{len(set(invalid_way_ids))} construction ways had no recoverable geometry; IDs are listed in the manifest")
    return ConstructionMetroImportResult(geojson_path, manifest_path, ImportReport(str(pbf_path), relations.relations_read, len(features), len(invalid_way_ids), tuple(warnings)))
