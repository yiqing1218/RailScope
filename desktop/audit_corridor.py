...
def audit(library, start_query, line_query, end_query):
    ...
    try:
        ...
        normalized = library.normalize_sequence(sequence)
        path = library.resolve(normalized)
        report["corridor"] = {
            "status": "unique_continuous_geometry", "start": start, "line_id": line, "end": end,
...
def main():
    ...
    report = audit(DiskRailLineLibrary(index, metadata=metadata), args.start, args.line, args.end)
    ...
    if args.report:
        ...
        print(output)
    return 0 if report["corridor"]["status"] == "unique_continuous_geometry" else 1
