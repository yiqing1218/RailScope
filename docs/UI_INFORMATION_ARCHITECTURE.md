...
- 地铁线路按省、市、线路分组；站点另按省、市、线路列出。同一换乘站可出现在多条线路下，但每个入口指向同一个稳定车站实体。
...
右侧提供详细概览、完整原始属性、工作区名称/目录编辑和复制按钮。对象标题使用简洁名称，稳定编号在概览中单列。地图点击要素会同步定位左侧目录；演示车辆的距离和状态实时刷新。

国铁图层有铁路线、在建铁路、车站轮廓、线路所及道岔四个总开关。线路目录按全国铁路业务分类列出整条线路；车站目录按省、市列出车站实体与控制点。主地图上方的统一搜索框用类型下拉框选择城市、地铁线路/站点、铁路线、铁路车站或控制点。
